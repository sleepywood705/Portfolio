import "../../../scss/Addict_Offline.scss";


const storeData = [
  {
    title: "OLIVE YOUNG",
    locations: [
      {
        name: "전국 올리브영",
        address: "",
        phone: "",
      },
    ],
  },
  {
    title: "CHICOR",
    locations: [
      {
        name: "강남역점",
        address: "서울시 서초구 강남대로 441 서산빌딩",
        phone: "02-3495-7600",
      },
      {
        name: "신세계 강남점",
        address: "서울시 서초구 신반포로 176",
        phone: "02-2150-2393",
      },
      {
        name: "신세계 센텀시티점",
        address: "부산광역시 해운대구 센텀4로 15, B2",
        phone: "051-745-2020",
      },
      {
        name: "광주점",
        address: "광주광역시 서구 죽봉대로 61 지하1층",
        phone: "062-360-1023",
      },
      {
        name: "안성점",
        address: "경기 안성시 공도읍 서동대로 3930-39",
        phone: "031-8092-1496",
      },
    ],
  },
  {
    title: "THE HYUNDAI BECLEAN",
    locations: [
      {
        name: "더현대 서울점",
        address: "서울시 영등포구 여의대로 108, B2 비클린",
        phone: "02-3277-0891",
      },
    ],
  },
  {
    title: "DUTY FREE",
    locations: [
      {
        name: "현대백화점 면세점 무역센터점",
        address: "서울시 강남구 테헤란로 51, 9F",
        phone: "1811-6688",
      },
    ],
  },
];

export function AddictOfflinePage() {
  return (
    <section id="AddictOfflinePage">
      <img
        src="https://media.11corporation.com/cdn/addict/shopby/store/20240717/store-pc.jpg"
        alt="img"
      />
      {storeData.map((store, index) => (
        <div className="wrap" key={index}>
          <h3>{store.title}</h3>
          {store.locations.map((location, locIndex) => (
            <div key={locIndex}>
              <span>{location.name}</span>
              {location.address && <p>{location.address}</p>}
              {location.phone && <p>{location.phone}</p>}
            </div>
          ))}
        </div>
      ))}
    </section>
  );
}
